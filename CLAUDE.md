...
**2026-05-22 — Durcissement sécurité + polissage du feed Discover (session autonome).** Quatre chantiers en une passe. **(1) JSON-LD anti-XSS** (item ouvert #4, stored-XSS MEDIUM) : les 39 blocs `<script type="application/ld+json">` sérialisaient via `JSON.stringify` nu — une question de sondage ou un nom de profil contenant `</script>` cassait la balise → XSS stockée. Nouveau helper `lib/json-ld.ts` (`jsonLdHtml`, échappe `<` `>` `&` U+2028 U+2029), balayé sur 30 fichiers via 4 agents parallèles. **(2) DB** : mig 030 fixe le `search_path` des 5 fonctions signalées par l'advisor (`poll_is_noise`, `sweep_dead_polls`, `pick_random_track`, `ask_recent_count`, `is_noisy_question`). **(3) Feed Discover** : retour haptique vote/révélation/skip (`lib/haptics.ts`, no-op iOS, aussi câblé sur le `PollCard` du home) ; animation d'entrée par carte (`disco-hero-in`/`disco-block-in` — question + zone de vote montent en fondu au snap) ; partage post-vote « verdict-first » (le lien part avec « 🌶️ REBEL · t'es dans les 12% » au lieu de la question nue). **(4) Image OG des sondages** : `app/[slug]/opengraph-image.tsx` rend les barres de % live + le total de votes — chaque partage de `/{slug}` devient un instantané de résultats. Vérifié : `tsc` clean, `/` `/discover` `/{slug}` + image OG rendent 200. Commit `a68e47b`.

**2026-05-22 — `/discover` refait en feed immersif plein écran (TikTok-like) — chantier #1 produit livré.** `/discover` était un snap-scroll avec une petite carte glass centrée + du vide autour. Refait : nouveau composant `app/discover/discover-card.tsx` — carte 100 dvh plein écran, **wash couleur vif par sondage** (gradient `paletteFor`/cosmétique + scrim sombre pour le contraste), question en hero centré, **pills de vote ancrées en bas** (zone pouce, ≥52 px). **Révélation après vote** : barres 0→%, bandeau verdict émotionnel animé (`verdict-reveal`) + toast points/coins. Auto-avance ~3 s après vote (annulée si swipe avant), swipe vertical snap + préchargement via IntersectionObserver, CTA création immersif tous les 5 sondages. `discover-feed.tsx` réécrit pour piloter `DiscoverCard` ; `page.tsx` allégé (chips SEO par carte retirées du feed — bruit visuel, gain crawl nul). `lib/reveal-copy.ts` extrait le verdict 8 langues (partagé avec `PollCard`). Zéro nouvelle clé i18n (réutilise `card.*`/`linker.share.label`/`share.copied`). Vérifié : `tsc` clean, `/discover` + `/` rendent 200. Commit `177c441`.

**2026-05-22 — `/vs` entièrement traduit dans les 8 langues.** Le hub de comparatifs `/vs` (980 paires uniques, 10 catégories) était traduit en es/it/de mais pt/ja/zh restaient incomplets — et même es/it/de n'étaient pas tous câblés dans `lib/seo/vs/translations/index.ts`. Terminé : pt/ja/zh complétés via ~24 agents parallèles (~1950 paires), `index.ts` câble les 43 packs de traduction. Les 980 paires rendent désormais dans les 8 langues (en/fr/es/it/pt/de/ja/zh) → ~7 840 pages `/vs` localisées, toutes au sitemap avec hreflang. Vérifié : `tsc` clean, couverture 980/980 par langue. Commits `f1e3f95` (es/it/de) + `040db20` (pt/ja/zh). Détail : le contenu `/vs` a 19 slugs dupliqués entre catégories (ex. `react-vs-vue` rangé en tools ET tech) — sans gravité, le loader dé-duplique par slug (980 uniques sur 1000 entrées brutes).
...
## 🎯 Cœur produit — feed Discover immersif ✅ LIVRÉ (2026-05-22)
Le chantier #1 est fait : `/discover` est une expérience TikTok-like pour les opinions (cartes 100 dvh plein écran, vote pouce ancré en bas, révélation barres + verdict animé, swipe snap + préchargement, CTA tous les 5). Détail dans « Where we left off » + commit `177c441`. Positioning : « moomz — the opinion engine ». Toujours décliné : follower system / commentaires (tuent l'anonyme-rapide). Itérations livrées (2026-05-22, commit `a68e47b`) : haptique au vote, animation d'entrée des cartes, image OG résultats-live, partage post-vote verdict-first. Restantes : double-tap pour voter, image OG dédiée au verdict perso.
...
- **Reste de la mega-session précédente** (`f827679` `d02b21b` `33e7d07` `fbbbbc1`, voir changelog) : #4 escaping JSON-LD `</script>` **réglé** (2026-05-22, commit `a68e47b` — helper `lib/json-ld.ts`) ; reste #5 RPCs gameplay anon-exécutables (mitigé, par design pour le flow anon).
...
- **SEO surface** (~2000+ routes): `/idees` `/ideas` `/guides` `/blog` (landing pages), `/mot` `/word` + `/topic/[locale]` (keyword pages, ~990 across all 8 langs, poll interlinking), `/compare` (30) `/vs` (980 comparison pairs, all 8 langs) `/template` (132, all 8 langs) `/quiz` (~60) `/citations` `/quotes` `/citation/[locale]`, and 9 utility tools `/convertisseur /meteo /heure /jours-feries /crypto /definition+/define /cosmos /recettes /astro` under the `/outils` hub. Data in `lib/seo/**` + `lib/tools/**` + `lib/quizzes/**`, mostly JSON loaded via `fs.readdirSync`. All wired into `app/sitemap.ts` + `app/seo-footer.tsx`; route names guarded in `RESERVED_USERNAMES`.
...
- **Fake votes removed (2026-05-22, mig 028).** The `fake_vote_burst` bot + its crons are gone — moomz now shows real votes only. Decision reversed: no bot seeding.
...
## Database
Postgres tables incl. `polls`, `votes`, `profiles`, `ask_questions`, `daily_moomz`, `tracks`, `push_subscriptions`, `coin_transactions`, `achievement_defs`. Public views `polls_with_stats`, `polls_trending`, `profiles_public`, `ask_questions_public`, `votes_world_24h`. Migrations in `supabase-migrations/` numbered 001→030 (auth, polls-cleanup, seed, ask, vote-country, security-hardening, anti-noise/bot-cap, achievement/coins, predictions, boost, shop, poll-translations, function-search-path). Apply via the Supabase MCP (`apply_migration`/`execute_sql`) or the PAT. The public views (`polls_trending`, `profiles_public`, `polls_with_stats`, `ask_questions_public`, `votes_world_24h`) are all `security_invoker=true` (verified 2026-05-22).
...
- **2026-05-22** — Sécurité + polissage Discover (commit `a68e47b`) : helper `lib/json-ld.ts` anti-XSS balayé sur 39 blocs JSON-LD / 30 fichiers ; mig 030 `search_path` de 5 fonctions ; haptique vote/révélation/skip (`lib/haptics.ts`) ; animation d'entrée des cartes Discover ; partage post-vote verdict-first ; image OG des sondages avec barres de % live + total de votes.
- **2026-05-22** — `/discover` refait en feed immersif plein écran TikTok-like : nouveau `discover-card.tsx` (carte 100 dvh, wash couleur par sondage, vote pouce en bas, révélation barres + verdict animé, auto-avance ~3 s, CTA tous les 5). `lib/reveal-copy.ts` extrait (partagé avec PollCard). Commit `177c441`.
- **2026-05-22** — `/vs` traduit dans les 8 langues : pt/ja/zh complétés (~1950 paires via ~24 agents parallèles), `lib/seo/vs/translations/index.ts` câble les 43 packs. 980 paires × 8 langues ≈ 7 840 pages `/vs` localisées au sitemap. Commits `f1e3f95` `040db20`.
- **2026-05-22** — URLs SEO localisées (`/en/courses` etc.) ; votes fake supprimés (mig 028, −23.8k) ; auto-traduction des sondages (mig 029, MyMemory) ; anti-keysmash renforcé. Commits `c9edbdb` `1570b80` `511b602`.
- **2026-05-21** — Migration i18n vers URLs par langue (Stages 1-2 + 4) : `middleware.ts` réécrit `/{loc}/{route SEO}` → route existante via header `x-moomz-locale` ; toute la surface SEO indexable par langue. Fix racine `force-static` (22 pages) + filtrage hubs par locale. `/formation` entièrement traduit en anglais (1000 modules, 18 agents). Sitemap per-locale + hreflang + canonical 54 pages. Commits `288f4c2`→`ded7478`. Migration i18n fonctionnellement complète. Reste : liens nav internes (micro-optim), Stage 5 (contenu es/it/pt/de/ja/zh).
