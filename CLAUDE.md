...
**2026-05-24 — Vote 1-clic + fix double-saut feed + warnings Search Console.** Utilisateur signalait dans la home : (a) après un vote ça « sautait au prochain puis direct encore au prochain » (b) il fallait 2 taps pour voter.
- **Vote en 1 clic** : suppression du double-tap `armed` state dans `discover-card.tsx` — `vote(i)` est appelé directement au 1er clic.
- **Fix double-saut** : suppression de l'auto-advance `setTimeout(3000)` post-vote. C'était la cause : le timer pouvait se superposer à un swipe manuel → 2 cartes en avant d'un coup. L'utilisateur swipe désormais lui-même quand il a lu le verdict (plus dans l'esprit Gen Z snap-scroll, pas d'auto-play forcé). Removed `onVoted` prop, `scrollToNext` helper, `isVisibleRef`, `disarmTimerRef`.
- **Search Console — QAPage** (`app/[slug]/page.tsx`) : ajout `author` + `datePublished` + `url` sur mainEntity (Question) ET sur chaque `suggestedAnswer`. Auteur = Organization moomz (polls anonyme-friendly). Règle les 4 warnings « Champ X manquant dans mainEntity / suggestedAnswer ».
- **Search Console — Recipe** (`app/recettes/[ingredient]/page.tsx`) : retire `@type:"Recipe"` des items ItemList. TheMealDB filter.php ne renvoie que name/image/url (pas de nutrition/prepTime/ingredients/instructions). Plus de schéma Recipe avec champs obligatoires manquants. Les 12 warnings disparaissent. Commit `7ff98ec`.

**2026-05-23 — La vraie cause du « home charge à l'infini » identifiée et corrigée.** Le 1er essai (`<Suspense>` + skeleton) a empiré le truc en prod (skeleton bloquait, jamais remplacé) → revert. Vraie cause : la home SSR émettait **95 sections plein écran** (80 polls + ~15 CTA) = 443 KB de HTML + hydratation de 80 `<DiscoverCard>` (chacun avec IntersectionObserver, multiples states/effects, Confetti, …). Sur mobile mid-range, ça fait 5-10s de main-thread bloqué → ressenti « infini ».
- **Fix #1** : `FEED_LIMIT = 25` dans `app/_feed/immersive-feed.tsx` (passé en 2e arg de `buildFeed`). Le lake fait 5k+ polls, RPC sample aléatoire, donc chaque ouverture donne un batch frais. 25 = largement assez pour une session snap-scroll Gen Z. HTML divisé par ~3.5.
- **Fix #2** : virtualisation côté client dans `discover-feed.tsx` — `RENDER_RADIUS = 2`, seules les cartes dans `±2` de `activeIdx` sont vraiment montées (`<DiscoverCard>` réel) ; les autres = `<section>` vides de 100dvh (la physique snap reste intacte). Au scroll, `activeIdx` update → mount des suivantes avant qu'elles entrent dans le viewport. Hydratation : ~5 cartes au lieu de 25.
- Pastille « ▶ Vibe radio » idle remise bas-droite (label i18n 8 langues) — relançable depuis toutes les pages.
- Time-to-interactive cold-mobile : devrait passer de « infini » à 1-2s.
