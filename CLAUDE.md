...
- **Reste à faire** : Stage 3 (sitemap per-locale + `hreflang` + `canonical` + liens internes locale-aware — dépend d'uniformiser proprement les routes jumelles) ; Stage 4 (traduire le contenu `/formation` — 1000 modules 100 % FR, aucun champ `locale`) ; Stage 5 (contenu es/it/pt/de/ja/zh). Plan détaillé : `~/.claude/plans/fluffy-launching-rivest.md`.
...
- **2026-05-21** — Migration i18n vers URLs par langue (Stages 1-2) : `middleware.ts` réécrit `/{loc}/{route SEO}` → route existante via header `x-moomz-locale` ; toute la surface SEO indexable par langue. Fix racine `force-static` (22 pages) + filtrage hubs par locale. Commits `288f4c2` `78b179f` `4f575f8` `87c5b3d` `e766e9b`. Reste : Stage 3 (sitemap/hreflang/liens), Stage 4 (traduction `/formation`).
