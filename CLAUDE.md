...
Le chantier #1 est fait : `/discover` est une expérience TikTok-like pour les opinions (cartes 100 dvh plein écran, vote pouce ancré en bas, révélation barres + verdict animé, swipe snap + préchargement, CTA tous les 5). Détail dans « Where we left off » + commit `177c441`. Positioning : « moomz — the opinion engine ». Toujours décliné : follower system / commentaires (tuent l'anonyme-rapide). Itérations possibles : haptique au vote, transition inter-cartes, image OG partageable du verdict, double-tap pour voter.
