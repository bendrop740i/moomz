...
**M1 fondation shippée ce passage** (data layer + moteur — pas encore câblé ni d'UI) :
...
**M1 RESTE À FAIRE** : étendre `apply_vote_streak` pour renvoyer les compteurs bruts ; câbler `castVote`/`createPoll`/`claimUsername` → `evaluate()` + RPC `claim_achievements` ; tracker les nouveaux compteurs dans les actions concernées (votes par heure du jour/week-end, ASK dans `ask-actions.ts`, quiz, exploration/skip, `received_votes_total`, `topic_polls`…) ; UI page haut faits (grille par catégorie, 8 langues) + solde de coins dans le StreakHUD.
