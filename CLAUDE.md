...
**2026-05-23 — Hotfix « home charge 3-4 fois avant de s'afficher » : streaming SSR via `<Suspense>`.** Le `<ImmersiveFeed/>` (async, await `buildFeed` Supabase RPC) bloquait l'envoi du HTML jusqu'à résolution → fenêtre blanche multi-secondes à chaque cold-start qui se ressentait comme plusieurs cycles de chargement. Maintenant entouré d'un `<Suspense fallback={<FeedSkeleton/>}>` : chrome + skeleton card plein écran (`app/_feed/feed-skeleton.tsx`) partent dans le premier chunk HTML, le feed réel arrive en streaming.
