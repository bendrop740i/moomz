...
**Shipped & live:** parametric achievement engine — `lib/achievements/{families.ts,engine.ts,i18n.ts}`, 50 families × tiers → **418 achievements**, 8-lang via `{n}`/`{topic}` templates. DB migrations 016/016b/017/018 applied: ~25 counter columns on `profiles`, `coin_transactions` ledger (idempotent), `achievement_defs` catalogue (418 rows), RPCs `apply_coins` (spend-only), `claim_achievements` (server re-validates — client can't cheat), `get_achievement_stats`. The vote→achievements→coins loop runs live in `castVote` (try/catch — never blocks a vote). Page `/haut-faits` shows progress; StreakHUD shows 🪙 balance. ~120 achievements already unlockable.

**M1 remaining:** track the rest of the counters in their actions (night/dawn/weekend votes + `active_days` + `topic_votes` + `rebel_streak` in `apply_vote_streak`; `topic_polls` + `received_votes_total` in `createPoll`/`castVote`; ASK counters in `ask-actions.ts`; quiz; `polls_seen` on skip/discover). Then start M2.
