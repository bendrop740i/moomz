...
**2026-05-21, i18n EN sweep — fin des fuites de français (`7973593`).** User: "trop de chose en français" sur la version anglaise. Balayage complet de `app/` avec 7 agents parallèles (un par surface), 59 fichiers corrigés. Trois patterns tués : (1) chrome français hardcodé — JSX, `aria-label`, `title`, `placeholder`, erreurs/toasts — câblé à la locale via tables `Record<Locale,…>` locales ou `useT()`/`getLocale()` (music player, nav, poll-card, daily-card, create-form, profile-form, dashboard `/me`, vote-client, linker, made-with-moomz, achievement-toast, manifest, OG images…) ; (2) `export const metadata` français statique → `generateMetadata()` locale-aware avec tables META 8 langues (layout, not-found, discover, music, play, haut-faits, boutique, mes-votes/sondages, explore, blog, guides, read, quiz, formation, tous les outils) ; (3) tables partielles (ternaires fr/en, langues manquantes) complétées aux 8 langues. **Règle confirmée :** routes intentionnellement FR (`/mot` `/idees` `/definition` `/citations`) gardent leur français — elles ont des jumelles EN (`/word` `/ideas` `/define` `/quotes`). Le contenu user (questions de sondage) n'est jamais traduit. Archi i18n inchangée (correcte) : 1 URL/page, pas de préfixe locale, locale = cookie + Accept-Language. `tsc --noEmit` clean.
...
- **2026-05-21** — i18n EN sweep : fin des fuites de français sur la version anglaise (7 agents parallèles, 59 fichiers). Chrome hardcodé câblé à la locale, `metadata` statique → `generateMetadata()` 8 langues, tables partielles complétées. Routes FR voulues (`/mot` `/idees` `/definition` `/citations`) gardées en français. Commit `7973593`.
