...
- **Moteur de haut faits paramétrique** — `lib/achievements/{families.ts, engine.ts, i18n.ts}`. ~24 familles (Voter ×N, Série, Daily, Habitué, Créateur, Viral, Rebelle, Majorité, Points, Coins, Dépensier, Collectionneur, Pronostics, Oracle + 10 familles par-thème) × paliers → **229 haut faits générés**, traduits **8 langues** via gabarits `{n}` (≈25 familles traduites au lieu de 8000 strings). `evaluate(stats, owned)` pur. `coinRewardFor()` = courbe log douce (~4→62 coins) : trickle volontaire pour ne pas inonder l'économie.
- **DB migrations 016 + 016b appliquées live (HTTP 201)** : colonnes `profiles` (`coin_balance`, `coins_earned_total`, `coins_spent_total`, `active_days`, `last_active_date`, `best_poll_votes`, `topic_votes` jsonb, `predictions_made/won`, `achievements_v2` jsonb). Ledger `coin_transactions` (audit + idempotence via unique index `(profile_id,reason,ref)`). Catalogue `achievement_defs` (229 rows). RPCs `resolve_profile_id`, `apply_coins` (**SPEND only** — ne peut pas minter), `claim_achievements` (**re-valide** chaque haut fait contre les vrais compteurs + `coin_reward` canonique → client ne peut pas tricher même en role anon). Seed généré par `scripts/gen-achievement-defs.mjs`.
- **Honnêteté sur le chiffre** : "1000" n'était pas réaliste sans remplissage bidon. Livré **~229 de qualité**, moteur scalable (ajouter familles/paliers = +haut faits gratuits). Ancien `lib/achievements.ts` (11 badges legacy) intact.

**M1 RESTE À FAIRE** : étendre `apply_vote_streak` pour renvoyer les compteurs bruts ; câbler `castVote`/`createPoll`/`claimUsername` → `evaluate()` + RPC `claim_achievements` ; tracker `active_days`/`best_poll_votes`/`topic_votes` ; UI page haut faits + solde de coins dans le StreakHUD.

**2026-05-20, auth bug fixed — logged-in users could not reserve a username.** User reported "ya un bug avec l'auth" with the error text *"An error occurred in the Server Components render. The specific message is omitted in production builds…"*.
