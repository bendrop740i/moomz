...
**2026-05-20, CRITICAL feed-empty bug fixed.** User reported people "can't really vote on polls created by other users" — the core of the app. **Root cause**: `app/_home/trending-feed.tsx` and `app/discover/page.tsx` both `SELECT last_vote_at` from the `polls_trending` view, but migration `016-views-lang-column.sql` had recreated that view via `CREATE OR REPLACE` and silently dropped `last_vote_at` (migration 005 had restored it earlier). PostgREST rejects a SELECT naming a missing column with **HTTP 400** → `data` was `null` → `getTrending()` returned `[]` → **both the home Trending feed and the Discover feed rendered empty**, so there were no other-people's polls to vote on. Voting itself + `apply_vote_streak` + the anon vote INSERT were all verified healthy — the feed just had nothing in it. **Fix**: migration `025-restore-trending-last-vote-at.sql` — `DROP VIEW ... CASCADE` (zero dependents, verified) + recreate `polls_trending` WITH (security_invoker=true), `last_vote_at` added back as `max(votes.created_at)` in the `vote_counts` CTE; all other columns/scoring byte-identical. Applied live via PAT (HTTP 201). Verified: both feed queries now return HTTP 200 with 40 rows incl. 32 anon-created (null-profile) real polls. Also added `create` to `RESERVED_USERNAMES` in `app/actions.ts` (`search`/`explore` were already there). `npx tsc --noEmit` clean for touched files.
