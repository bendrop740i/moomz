...
- **Stage 3 — sitemap per-locale + hreflang (`38e408c`)** : `app/sitemap.ts` réécrit — chaque hub SEO émet ses 8 URLs `/{loc}/<route>` reliées par `hreflang` (`alternates.languages`) ; détails de contenu à leur locale d'autorat ; outils avec hreflang 8 langues. ~14k URLs, ~4,9k liens hreflang. Google découvre les 8 langues directement.
- **Reste à faire** : fin du Stage 3 — `canonical` par page encore en URL non-préfixée (~30 pages SEO l'ont en dur ; `hreflang` du sitemap couvre l'essentiel) + liens de nav internes encore vers les URLs nues (marchent via 301, juste un saut) ; Stage 5 (contenu es/it/pt/de/ja/zh — aujourd'hui ces locales retombent sur EN/FR). Plan : `~/.claude/plans/fluffy-launching-rivest.md`.
