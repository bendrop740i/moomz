...
**2026-05-20, monétisation — décision stratégie + M1 fondation (coins + moteur de haut faits) (latest).** Discussion monétisation. **Décision actée** : économie de **coins** (monnaie virtuelle) comme colonne vertébrale — plusieurs faucets (streaks, Daily, haut faits, predictions gagnées), sinks = boost de sondage + predictions + cosmétiques. **Plan M1→M4** : M1 fondation coins + haut faits paramétriques *(en cours)*, M2 predictions (parier des coins — légal car **aucun cashout** + jouable avec coins gratuits → pas un jeu d'argent), M3 boost de sondage + boutique de coins, M4 Stripe (achat de coins). Pub sur les pages SEO = revenu passif en parallèle. Démasquage ASK type NGL **volontairement écarté** (FTC a infligé $5M à NGL — bots + indices bidons ; moomz a déjà des bots → même piège).

**M1 fondation — moteur + data layer** :
- **Moteur de haut faits paramétrique** — `lib/achievements/{families.ts, engine.ts, i18n.ts}`. **50 familles** (30 core sur 12 domaines : votes, votes par heure/week-end, séries, fidélité, ancienneté, création, performance, rebelle, économie, ASK, exploration, pronostics, méta + 10 familles vote-par-thème + 10 création-par-thème) × paliers → **418 haut faits générés**, traduits **8 langues** via gabarits `{n}`/`{topic}` (≈30 familles traduites au lieu de 8000 strings). `evaluate(stats, owned)` pur. `coinRewardFor()` = courbe log douce (~4→62 coins) : trickle volontaire pour ne pas inonder l'économie.
- **DB migrations 016 + 016b + 017 appliquées live (HTTP 201)** : ~25 colonnes compteurs sur `profiles` (`coin_balance`, `coins_earned/spent_total`, `active_days`, `best_poll_votes`, `received_votes_total`, `night/dawn/weekend_votes`, `rebel_streak_top`, `questions_asked/answered/received`, `quizzes_completed`, `polls_seen`, `cosmetics_unlocked`, `polls_boosted`, `prediction_*`, `topic_votes`/`topic_polls` jsonb, `achievements_v2` jsonb). Ledger `coin_transactions` (audit + idempotence via unique index `(profile_id,reason,ref)`). Catalogue `achievement_defs` (418 rows). RPCs `resolve_profile_id`, `apply_coins` (**SPEND only** — ne peut pas minter), `claim_achievements` (**re-valide** chaque haut fait contre les vrais compteurs + `coin_reward` canonique → client ne peut pas tricher même en role anon). Seed généré par `scripts/gen-achievement-defs.mjs` (prune les familles renommées).
- **Sur le chiffre** : user a demandé de gonfler "de qualité, tous les domaines" → 418 haut faits légitimes sur 12 domaines (pas de remplissage bidon). Moteur scalable. Ancien `lib/achievements.ts` (11 badges legacy) intact, coexiste.

**M1 câblage shippé (passage suivant)** : la boucle vote → haut faits → coins **tourne en live**.
- **Migration 018** appliquée : `profile_metrics(profiles)` (snapshot métrique canonique partagé), `get_achievement_stats(user_id, claim_token)` (lecture seule : metrics + owned + solde), `claim_achievements` recréée via le helper. `apply_vote_streak` **pas touchée** (zéro risque sur le vote).
- **`castVote`** (`app/actions.ts`) : après `apply_vote_streak`, appelle `get_achievement_stats` → `evaluateFromMetrics()` (nouveau, dans `engine.ts`) → `claim_achievements`. Tout dans un `try/catch` — un échec ne bloque jamais le vote. Renvoie `newAchievements` + `coins {balance,gained}`. Pose un cookie `moomz_coins` (lisible JS).
- **UI** : `app/streak-hud.tsx` affiche le solde 🪙 (event `moomz:coins` + cookie), cliquable → `/haut-faits`. `vote-client.tsx` + `poll-card.tsx` émettent `moomz:coins`. Nouvelle page **`/haut-faits`** : grille par catégorie, progression par famille (palier actuel → prochain + barre + coins), 8 langues, `noindex`. Usernames réservés : `haut-faits`/`achievements`/`coins`/`wallet`.
- **Vérifié end-to-end** (profil test jetable) : claim accorde 28 coins pour 5 haut faits gagnés, **rejette** les ids non-mérités (`votes.25` à 10 votes) et bidon, idempotent, ledger OK.
- **Marchent en live dès maintenant** (compteurs déjà trackés par `apply_vote_streak`/`createPoll`) : familles `votes`, `streak`, `daily`, `rebel`, `majority`, `points`, `polls`, `veteran`, `coins`, `collector` ≈ 120 haut faits déblocables en votant/créant.

**M1 RESTE À FAIRE** : tracker les compteurs restants dans leurs actions (votes nuit/aube/week-end + `active_days` + `topic_votes` + `rebel_streak` → dans `apply_vote_streak` ; `topic_polls` + `received_votes_total` → `createPoll`/`castVote` ; ASK → `ask-actions.ts` ; quiz ; `polls_seen` → skip/discover). Puis M2 predictions.

**2026-05-20, auth fixes — username-reserve crash + Google sign-in removed.** User reported "ya un bug avec l'auth" with the error text *"An error occurred in the Server Components render. The specific message is omitted in production builds…"*.

**Root cause.** `profiles.claim_token` was declared `NOT NULL` (no default). But `claimUsername()` in `app/actions.ts` (line ~265) intentionally inserts `claim_token: null` for Supabase-Auth session users — their identity is `user_id`, not the legacy anonymous `claim_token`. So any **logged-in user with no profile** who tried to reserve a username hit `INSERT … profiles` → `23502 null value in column "claim_token" violates not-null constraint`. `claimUsername()` re-threw it; in production Next.js **sanitizes server-action errors** into the generic *"An error occurred in the Server Components render…"* message, and `app/me/profile-form.tsx`'s form-action `catch` does `setError(e.message)` → that generic text rendered in the form's red error box. That's the exact symptom the user saw.

**Diagnosis path.** Reproduced anonymous `/login` `/me` `/auth/callback` (all 200, clean) → minted a test session for `bendrop740i@gmail.com` via Management-API `generate_link` + `verify`, curled every authed page (all 200) → found the user has **no `profiles` row** linked to their `user_id` → inspected `profiles` columns → spotted `claim_token NOT NULL` vs the code passing `null` → confirmed live with a rollback'd `INSERT` (`23502`). Supabase auth-service logs were all `200` (service healthy); Vercel runtime logs unavailable (free-tier retention).

**Fix.** `supabase-migrations/017-claim-token-nullable.sql` — `alter table public.profiles alter column claim_token drop not null;`. **Applied live to prod DB** (verified: rollback'd `INSERT` with null `claim_token` now succeeds). No code change needed — `claimUsername()` was already correct, the column just had to accept NULL. **No redeploy required** — the prod app already passes `null`; the schema change makes it valid immediately. Migration file committed by a parallel session.

**Google sign-in removed (user request — "enleve google ca me casse les couilles").** Google OAuth was enabled in Supabase but with **placeholder credentials** (`client_id: 12345-xxxxxxxxxxxx.apps.googleusercontent.com`, fake 64-char secret) → the button always failed with `invalid_client`. After walking the user through a Google Cloud Console setup they decided to drop it. Done: removed the Google `<button>` + OR-separator from `app/login/login-form.tsx` (also dropped now-unused `google`/`orSeparator` props + `pendingGoogle` state; props removed from `app/login/page.tsx` call site), and set `external_google_enabled: false` in Supabase auth config via the Management API. Commit `7ecb9ec` pushed to main. **Magic-link is now the sole auth path** — and it's fully production-ready: Resend SMTP wired (`smtp.resend.com`, sender `hello@moomz.com`), the `moomz.com` sending domain is DNS-verified (DKIM `resend._domainkey` + SPF `send.moomz.com` + feedback MX all present → not spam), and a branded dark-mode HTML email template (`supabase-migrations/email-magic-link.html`, "Ton lien magique ✨") is installed live. The `auth.login.google`/`auth.login.or` i18n keys were left in the dictionaries (harmless unused data; re-addable if Google returns later). To re-enable Google later: create a real OAuth client in Google Cloud Console (redirect URI **must** be `https://tpncpfatthgssttzrchu.supabase.co/auth/v1/callback`), paste creds into Supabase, flip `external_google_enabled` back on, restore the button.

**Still open / not done this pass:** security advisors flagged that `polls_trending` and `profiles_public` are back to `SECURITY DEFINER` (migration 012 set them to INVOKER — a later migration reverted them). Not touched here; flag for a follow-up security pass.

**2026-05-20, citation/quote SEO pack — 128 pages, 1024 quotes, 8 langs.** User asked for quote pages linked from polls when someone votes, in every language ("des pages citation aussi, genre 1000 citations et que ce soit linker quand quelqu'un vote pr ça dans toutes les langues, met plusieurs agents"). Shipped via **8 parallel background agents** (one per language) + a main-thread infra/wiring pass.
