  ...
  return (
    <div className="space-y-8 fade-up">
      <header className="space-y-2">
        <h1 className="text-4xl font-bold tracking-tight">Mes sondages</h1>
        <p className="text-white/50 text-sm">
          {polls.length > 0
            ? `${polls.length} sondage${polls.length > 1 ? "s" : ""} créé${polls.length > 1 ? "s" : ""} · ${totalVotes} vote${totalVotes > 1 ? "s" : ""} reçu${totalVotes > 1 ? "s" : ""}`
      ...
      {polls.length === 0 ? (
        <div className="glass rounded-3xl p-8 text-center space-y-4">
          ...
          <p className="text-white/60">Crée ton premier moomz et partage-le.</p>
          <Link
            href="/"
