...
export default function MusicMiniPlayer() {
  const { current, isPlaying, isLoading, toggle, next, start } = useMusic();
    ...
    <div className="fixed bottom-20 right-3 z-30 select-none">
      {!current ? (
        <button
          onClick={start}
          disabled={isLoading}
          className="glass rounded-full shadow-lg shadow-black/30 px-3 py-2 flex items-center gap-2 text-xs hover:bg-white/[0.10] active:scale-[0.97] transition disabled:opacity-60"
          aria-label="Lancer la radio"
        >
          <span className="text-lg leading-none">🎵</span>
          <span className="font-semibold pr-1">{isLoading ? "…" : "Vibe radio"}</span>
        </button>
      ) : (
